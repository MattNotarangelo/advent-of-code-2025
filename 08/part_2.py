...
class Solution:
    ...
    def _check_final_condition(self, groups):
        return len(self.g) == len(self.nodes) and groups == 1

    def _return_final(self, n1, n2):
        return n1[0] * n2[0]
    ...
    def solve(self, s: str):
        ...
        for pair_index, (dist, i, j) in enumerate(self.distances):
            ...
            # if one is in a group
                ...
                self.g[i].add(j)
                self.g[j].add(i)
                if self._check_final_condition(groups):
                    return self._return_final(i, j)
